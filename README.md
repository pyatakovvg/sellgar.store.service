...
Создание, изменение и архивирование `store_product` вместе с его offers пишет
`store.product.updated` в `outbox_event` в той же транзакции. Outbox relay
публикует событие в RabbitMQ, повторяет неуспешные попытки с backoff и
восстанавливает записи, зависшие в `processing`. Получатель обязан выполнять
дедупликацию по `eventUuid`.
